...
> Update: 10/6/2023  New version of Sympy does not support non-Expr objects in a Matrix. Code to generate the circuit equations towards the end of the notebook produces an error. This line generates an error:  equ[i] = Eq(eq_temp,Z[i]). Fix has not been implemented.
